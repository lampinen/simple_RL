...
\begin{itemize}
\item A natural way to define ``best'' is based on expected return (which we call \textbf{value}). 
    \[V\onslide<3->{^\pi}(s) = \E{\text{Return} \, |\, s} = \E{\sum_{t=1}^\infty } \]
\item But  
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item<2-> Again, it's not supervised, nobody is telling you what the right move was.
